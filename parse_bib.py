    ...
    for entry in bib_database.entries:
        ...
        if os.path.isfile(filenm) and OVERWRITE == False:
            ...
            with open(filenm, 'w', encoding="utf8") as the_file:
                ...
                # if 'link' in entry:
                if os.path.isfile('static/pdfs/' + supetrim(entry['ID'])+'.pdf'):
                    the_file.write('url_pdf = "pdfs/'+supetrim(entry['ID'])+'.pdf"\n')
                else:
                    print('no pdf found for ' + entry['ID'])
